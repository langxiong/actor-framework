...
public:
  ...
  explicit actor_system(actor_system_config& cfg,
                        detail::simulated_clock* clock = nullptr);
  ...
  spawn_impl(actor_config& cfg, Ts&&... xs) {
    ...
    if (CAF_UNLIKELY(clock_ != nullptr))
      cfg.flags |= abstract_actor::has_simulated_time_flag;
    if (cfg.host == nullptr)
...
private:
  ...
  /// Points to a fake clock when running CAF in simulation mode (usually null).
  detail::simulated_clock* clock_;
};
